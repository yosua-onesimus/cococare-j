...
//</editor-fold>
    ...
    protected void _initObjEntity() {
        objEntity = SnakeAndLadderMain.gameOption;
    }
    ...
    protected void _doSave() {
        if (_isValueValid()) {
            _getValueFromEditor();
            //
            List<Player> players = SnakeAndLadderMain.gameOption.getPlayers();
            for (int i = 0; i < 5; i++) {
            ...
            //
            if (updateCaller = SnakeAndLadderMain.saveGameOption()) {
                _doCloseScreen();
            } else {
    ...
    protected void _doUpdateTxtQuestionNumber() {
        ...
        questionNumber = Math.max(Math.min(questionNumber, Math.min(SnakeAndLadderMain.questions.size(), 25)), 0);
